...
public class TaskQ {
	 ...
	 */
	List<ComponentEntity> calculateList = componentService.calculateComponentQ();
	 ...
	 * 出库信息插入(沥青油，铣刨料,矿粉)
	 */
	List<AmpmonInfoEntity> ampList = ampmonInfoService.calculateComponentQ();
	 ...
	 */
		...
		/*石粉，米砂，瓜子片，玄武岩2#，玄武岩1#*/
		List<StockEntity> stockList = stockService.queryList();
		for(int i = 0;i<stockList.size()-3;i++){
			 if(i==3){
	             continue;//忽略i==3时的循环
	         }
			stockService.updateStockweight(stockList.get(i).getCodename(), stockList.get(i).getStockweight() - listA.get(i));
		}
		/*沥青油，铣刨料,矿粉*/
			stockService.updateStockweight(stockList.get(6).getCodename(), stockList.get(6).getStockweight() - (listB.get(0))/1000);
			stockService.updateStockweight(stockList.get(7).getCodename(), stockList.get(7).getStockweight() - (listB.get(1))/1000);
			stockService.updateStockweight(stockList.get(8).getCodename(), stockList.get(8).getStockweight() - (listB.get(2))/1000);
		/*成分4*/
			...
			stockService.updateStockweight(stockList.get(3).getCodename(), d);
	}
