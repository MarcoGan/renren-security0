$(function () {
	vm.getPlaceList();
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/weighingloading/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '毛重(t)', name: 'grossWeight', index: 'gross_weight', width: 50 }, 			
			{ label: '皮重(t)', name: 'tareWeight', index: 'tare_weight', width: 50 }, 			
			{ label: '净重(t)', name: 'netWeight', index: 'net_weight', width: 50 }, 			
...
var vm = new Vue({
	...
	data:{
		...
		place:null,
		placelist:{},
		weighingLoading: {placeIdList:null}
	},
	methods: {
		query: function () {
			vm.reload();
		...
		add: function(){
			...
			vm.title = "新增";
			vm.placelist = {};
			vm.weighingLoading = {placeIdList:null};
			this.getPlaceList();
		},
		...
		saveOrUpdate: function (event) {
			var url = vm.weighingLoading.id == null ? "sys/weighingloading/save" : "sys/weighingloading/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.weighingLoading),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/weighingloading/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/weighingloading/info/"+id, function(r){
                vm.weighingLoading = r.weighingLoading;
            });
		},
...
laydate.render({
	   ...
	   theme: '#17B3A3',
		  ...
		  vm.weighingLoading.dispatchDate = value
		  }
		});
laydate.render({
	   elem: '#sendtime',
	   ...
	   theme: '#17B3A3',
		  ...
		  vm.weighingLoading.sendtime = value
		  }
		});
laydate.render({
	   elem: '#startdate',
